...
### 앱 관리 업로드는 이미 설치된 채팅방에 전파되지 않는다

새 zip 을 올리면 조직 카탈로그의 항목만 갱신된다. **채팅방에 설치된 인스턴스는 그대로다** —
`version` 을 올려도 자동완성 메뉴에 예전 명령이 계속 뜬다.
해당 채팅방에서 앱을 제거하고 다시 추가해야 반영된다 (실행해서 확인함).

처음에는 이것을 "클라이언트 캐시" 로 진단했는데, 원인을 잘못 짚은 것이었다.
증상이 같아도 원인이 다르면 조치가 다르다 — 캐시라면 재시작으로 풀리지만,
설치 범위 문제는 **채팅방마다** 재적용해야 풀린다.
